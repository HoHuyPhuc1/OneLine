...
import static com.example.oneline.ChooseLevelActivity.*;
...
public class GraphicGame extends View {
    Paint p1,p2,p3;
    MediaPlayer TouchGame;
    MediaPlayer GameWin, GameOver;
    boolean TG = false, GW = false, GO = false;
    boolean dia = false;
    int x_mouse = 0;
    int y_mouse = 0;
    boolean win;
...
//        lv = Character.getNumericValue(a.charAt(a.length()-1));
        init(lv);
...
//        canvas.drawLine(_1.x,_1.y,_2.x,_2.y,p1);
//        canvas.drawLine(test.MangCanh.get(5).dau.x,test.MangCanh.get(5).dau.y,test.MangCanh.get(5).cuoi.x,test.MangCanh.get(5).cuoi.y,p1);
                ...
                if (c.dau == Start || c.cuoi == Start){
                    ...
                    if ((Start == c.dau && End == c.cuoi)||(Start == c.cuoi && End == c.dau)){
                        if (c.succ == false){
                            ...
                            Toast.makeText(Level1Activity.selfinter, "this line is already drawn", Toast.LENGTH_SHORT).show();
...
//            android.os.Process.killProcess(android.os.Process.myPid());
    ...
    public void openDialogLose(int gravity, Context context){
        ...
        builder.setMessage("Level Failed").setPositiveButton("Retry", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int id) {
                ...
                Level1Activity.selfinter.finish();
                Level1Activity.backgroundMusic.stop();
                GameOver.stop();
            }
        });
        builder.create().show();
    ...
    public void touch(){
        if (TG == false){
            TouchGame = MediaPlayer.create(Level1Activity.selfinter, R.raw.touch_game);
            TouchGame.setLooping(false);
            TouchGame.setVolume(10.0f, 10.0f);
            TG = true;
        }
    ...
    public void MusicWin(){
        if (GW == false){
            GameWin = MediaPlayer.create(Level1Activity.selfinter,R.raw.game_win);
            GameWin.setLooping(false);
            GameWin.setVolume(5.0f,5.0f);
            GW = true;
        }
    ...
    public void MusicLose(){
        if (GO == false){
            GameOver = MediaPlayer.create(Level1Activity.selfinter,R.raw.game_over);
            GameOver.setLooping(false);
            GameOver.setVolume(5.0f,5.0f);
            GO = true;
        }
